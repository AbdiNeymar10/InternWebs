...
import { createDepartment, updateDepartment, getDepartments } from '../services/departmentService';
...
const DepartmentForm = ({
    ...
    useEffect(() => {
        if (parentDeptId) {
            const parentDepartment = allDepartments.find(department => department.deptId === parentDeptId);
            if (parentDepartment) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setMessage({ text: 'Failed to save department. Please try again.', type: 'error' });
        }
    };
    ...
    return (
        <div className="modal fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className="modal-content bg-white rounded-lg shadow-lg p-6 w-full max-w-lg">
                <h2 className="text-xl font-bold mb-4">{dept ? 'Edit Department' : 'Add New Department'}</h2>

                {message.text && (
                    <div className={`mb-4 p-2 text-center ${message.type === 'success' ? 'bg-green-200 text-green-800' : 'bg-red-200 text-red-800'} rounded`}>
                        {message.text}
                    </div>
                )}
