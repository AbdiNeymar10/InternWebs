...
// Add prop types for DepartmentTreeSelect
interface DepartmentTreeSelectProps {
  selectedDeptId: string | number;
  onSelect: (deptId: string | number) => void;
}

// If this file is the DepartmentTreeSelect component, define it here:
...
// Export as named export to avoid multiple default exports
export { DepartmentTreeSelect };
interface IcfOption {
  id: number;
  icf: string;
}
  ...
  const [formData, setFormData] = useState<EmployeeFormData>({
    ...
    salary:
      ...
      "salary" in employeeData.salary
        ? String((employeeData as any).salary.salary ?? "")
      ...
      const fetchJobDetails = async () => {
        try {
          ...
          if (!response.ok) {
            ...
            throw new Error(
              `Network response was not ok: ${response.status} ${response.statusText}. Server said: ${errorText}`
            );
          }
      ...
      const fetchIncrementSteps = async () => {
        try {
          ...
          if (!response.ok) {
            ...
            throw new Error(
              `Failed to fetch increment steps. Status: ${response.status}, Message: ${errorText}`
            );
          }
