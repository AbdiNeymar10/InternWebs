...
const DelegationHistory = () => {
  ...
  const handleSearch = async (id: string) => {
    ...
    try {
      ...
      const delegatorResponse = await authFetch(
        `http://localhost:8080/api/hr-power-delegation/delegator/${id}`
      );
      const delegateeResponse = await authFetch(
        `http://localhost:8080/api/hr-power-delegation/delegatee/${id}`
      );
      const delegatorData = await delegatorResponse.json();
      const delegateeData = await delegateeResponse.json();

      // Combine results, removing duplicates by ID
      const combinedRecords = [
        ...delegatorData,
        ...delegateeData.filter(
          (d: any) => !delegatorData.some((dr: any) => dr.id === d.id)
        ),
      ].map((d: any) => ({
        id: d.id.toString(),
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-[#3c8dbc]/10">
                    <th className="text-left py-3 px-4 text-sm font-medium text-gray-600 border-r border-[#3c8dbc]/20">
                      ID
                    </th>
                    <th className="text-left py-3 px-4 text-sm font-medium text-gray-600 border-r border-[#3c8dbc]/20">
                      Delegatee ID
                ...
                <tbody>
                  {isLoading ? (
                    ...
                    <tr>
                      <td
                        colSpan={5}
                        className="py-4 px-4 text-sm text-red-500 text-center"
                      >
                        {error}
                      </td>
                    </tr>
                    ...
                    records.map((record) => (
                      <tr
                        key={record.id}
                        className="border-b border-[#3c8dbc]/10"
                      >
                        <td className="py-3 px-4 text-sm text-gray-700 border-r border-[#3c8dbc]/20">
                          {record.id}
                        </td>
                        <td className="py-3 px-4 text-sm text-gray-700 border-r border-[#3c8dbc]/20">
                          {record.delegateeId}
