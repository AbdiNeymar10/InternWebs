...
export default function UserList() {
  ...
  // Fetch user by empId
  const fetchUserByEmpId = async (empId: string) => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.get(
        `http://localhost:8080/api/auth/users/empid/${empId}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (response.data) {
        Swal.fire({
          title: "User Found",
          html: `<b>Name:</b> ${response.data.fullName}<br/><b>Email:</b> ${response.data.email}<br/><b>empId:</b> ${response.data.empId}<br/><b>Role:</b> ${response.data.role}`,
          icon: "info",
          confirmButtonColor: "#3c8dbc",
        });
      } else {
        Swal.fire({
          title: "Not Found",
          text: `No user found with empId ${empId}`,
          icon: "warning",
          confirmButtonColor: "#3c8dbc",
        });
      }
      ...
      Swal.fire({
        title: "Error!",
        text: `Failed to fetch user with empId ${empId}`,
        icon: "error",
        confirmButtonColor: "#3c8dbc",
      });
    }
      ...
      <div className="mb-6 flex items-center">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="pl-10 pr-4 py-2 border border-gray-300 rounded-l-md w-full focus:outline-none focus:ring-2 focus:ring-[#3c8dbc]"
            value={searchTerm}
        ...
        <button
          type="button"
          className="bg-[#3c8dbc] hover:bg-[#367fa9] text-white px-4 py-2 rounded-lg border-l-0 border border-gray-300 ml-2"
          style={{ marginLeft: "0.5rem" }}
          onClick={() => {
            if (searchTerm) fetchUserByEmpId(searchTerm);
          }}
        >
          Search
        </button>
